...
import './App.css'
import { CameraControls, OrbitControls, Stage, Stats } from '@react-three/drei'
...
export default function App() {
  ...
  return (
    <div className='relative w-screen h-screen'>
      <Canvas camera={mainCameraRef.current} eventPrefix='client' gl={{ preserveDrawingBuffer: true }}>
        <Suspense>
          <Stage position={[0, 0, 0]} adjustCamera>
            <EnvironmentWithSetting />
            <SceneCamera />
            <PlatformGroup />
            <OrbitControls camera={mainCameraRef.current} enablePan={false} enableRotate={false} enableZoom={false} />
          </Stage>
          <Stats />
        </Suspense>
      </Canvas>
        ...
        moveTo: folder(
          {
            ...
            'moveTo(…vec)': button(
              (get) => cameraControlsRef.current?.moveTo(...(get('moveTo.vec1') as [number, number, number]), true)
            )
          },
          { collapsed: true }
        ),
        setPosition: folder(
          {
            ...
            'setPosition(…vec)': button(
              (get) =>
                cameraControlsRef.current?.setPosition(...(get('setPosition.vec2') as [number, number, number]), true)
            )
          },
          { collapsed: true }
        ),
        setTarget: folder(
          {
            ...
            'setTarget(…vec)': button(
              (get) =>
                cameraControlsRef.current?.setTarget(...(get('setTarget.vec3') as [number, number, number]), true)
            )
          },
          { collapsed: true }
        ),
        setLookAt: folder(
          {
            ...
            'setLookAt(…position, …target)': button(
              (get) =>
